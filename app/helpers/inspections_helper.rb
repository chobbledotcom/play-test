...
module InspectionsHelper
  ...
  sig { params(inspection: Inspection).returns T::Array[Symbol] }
  ...
  sig { returns Symbol }
  def current_tab
    normalize_tab_param params[:tab]
  end

  sig { params(tab_param: T.nilable(String)).returns Symbol }
  def normalize_tab_param(tab_param)
    return :inspection if tab_param.blank?

    # Find the first matching tab (comparing strings to avoid to_sym on user input)
    matching_tab = Inspection::ALL_POSSIBLE_TABS.find { |tab| tab.to_s == tab_param }
    matching_tab || :inspection
  end

  sig { params(inspection: Inspection, tab: Symbol).returns T::Boolean }
  ...
  sig { params(inspection: Inspection, tab: Symbol).returns String }
  def tab_name_with_check(inspection, tab)
    name = t "forms.#{tab}.header"
    assessment_complete?(inspection, tab) ? "#{name} ✓" : name
  end

  sig { params(inspection: Inspection, current_tab: Symbol).returns T.nilable(T::Hash[Symbol, T.untyped]) }
  ...
  sig { params(inspection: Inspection, tab: Symbol).returns Integer }
