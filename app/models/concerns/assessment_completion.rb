  ...
  def incomplete_fields_grouped
    ...
    form_config.each do |section|
      ...
      section[:fields].each do |field_config|
        ...
        composite_fields = ChobbleApp::FieldUtils.get_composite_fields(field, partial)
    ...
    incomplete.each do |field|
      ...
      base_field = ChobbleApp::FieldUtils.strip_field_suffix(field).to_sym
      partial = field_to_partial[field] || field_to_partial[base_field]

      # Find all related incomplete fields for this base
      related = incomplete.select { |f| ChobbleApp::FieldUtils.strip_field_suffix(f).to_sym == base_field }
