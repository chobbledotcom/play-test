...
RSpec.feature "Bungee Run Inspection Workflow", type: :feature do
  ...
  scenario "complete bungee run inspection workflow" do
    unit = create_bungee_run_unit
    inspection = create_inspection_for_unit(unit)

    verify_correct_tabs_visible(inspection)
    fill_inspection_tab(inspection)
    fill_all_bungee_assessments(inspection)
    complete_inspection(inspection)
    verify_inspection_complete(inspection)
  end

  scenario "bungee run unit shows correct unit type" do
    visit units_path
    click_button I18n.t("units.buttons.add_unit")

    unit_data = SeedData.unit_fields.merge(name: "Test Bungee")
    unit_data.each do |field_name, value|
      fill_in_form :units, field_name, value
    end

    select I18n.t("units.unit_types.bungee_run"),
      from: I18n.t("forms.units.fields.unit_type")

    submit_form :units
    expect_units_message("created")

    unit = Unit.find_by!(name: "Test Bungee")
    expect(unit.unit_type).to eq("bungee_run")
  end

  scenario "bungee run inspection shows correct tabs" do
    unit = create(:unit, user: user, unit_type: :bungee_run)
    ...
    expect_assessment_tab("bungee")

    %w[user_height slide enclosed pat ball_pool
      catch_bed inflatable_game].each do |tab|
      expect_no_assessment_tab(tab)
    end
  end

  scenario "bungee run form renders all i18n fields" do
    unit = create(:unit, user: user, unit_type: :bungee_run)
    inspection = create(:inspection, user: user, unit: unit)

    visit edit_inspection_path(inspection, tab: "bungee")

    expect_form_matches_i18n("forms.bungee")
  end

  private

  def create_bungee_run_unit
    visit units_path
    click_button I18n.t("units.buttons.add_unit")

    unit_data = SeedData.unit_fields
      .merge(name: "Bungee Run Unit")
    unit_data.each do |field_name, value|
      fill_in_form :units, field_name, value
    end
    ...
    Unit.find_by!(name: "Bungee Run Unit")
  end

  def create_inspection_for_unit(unit)
    inspection = create(:inspection, user: user, unit: unit)
    expect(inspection.inspection_type).to eq("bungee_run")
    inspection
  end
  ...
  def verify_correct_tabs_visible(inspection)
    ...
    %w[enclosed pat slide user_height ball_pool
      catch_bed inflatable_game].each do |tab|
      expect_no_assessment_tab(tab)
    end
  end
  ...
  def fill_inspection_tab(inspection)
    ...
    field_data.except!(
      :has_slide, :is_totally_enclosed, :indoor_only
    )
  ...
  def fill_all_bungee_assessments(inspection)
    {
      ...
      "bungee" => SeedData.bungee_fields(passed: true)
    }.each do |tab, fields|
  ...
  def fill_results_tab(inspection)
    ...
    fill_in_risk_assessment("Unit passes all bungee checks")
    submit_form :results
