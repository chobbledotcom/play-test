...
RSpec.describe UserHeightAssessment, type: :model do
  ...
  describe "validations" do
    context "height measurements" do
      ...
      it "validates user_height is non-negative" do
        assessment.user_height = -1.0
        expect(assessment).not_to be_valid
        expect(assessment.errors[:user_height]).to include("must be greater than or equal to 0")
      end

      it "allows blank height measurements" do
        assessment.containing_wall_height = nil
        assessment.platform_height = nil
        assessment.user_height = nil
    ...
    context "when all required fields are present and valid" do
      it "returns true" do
        assessment.update!(
          ...
          user_height_comment: "Good conditions"
        )
