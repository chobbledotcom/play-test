...
RSpec.describe "Inspections Turbo Streams", type: :request do
  ...
  describe "Auto-save behavior" do
    let(:turbo_headers) { {"Accept" => "text/vnd.turbo-stream.html"} }

    it "handles rapid successive updates" do
      3.times do |i|
        patch inspection_path(inspection),
          params: {inspection: {comments: "Update #{i}"}},
          headers: turbo_headers

        expect(response).to have_http_status(:ok)
      end

      expect(inspection.reload.comments).to eq("Update 2")
    end

    it "preserves data across multiple assessment updates" do
      inspection.create_user_height_assessment!
      ...
      expect(assessment.containing_wall_height).to eq(1.5)
      expect(assessment.platform_height).to eq(2.0)
    end
  end
